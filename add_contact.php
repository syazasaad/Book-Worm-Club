...
if (isset($_POST["submit"])) {
    ...
    // Check if contact_name is not empty
    if (empty($contact_name)) {
        echo "Contact Name cannot be empty.";
    } else {
        $sql = "INSERT INTO contact (contact_id, contact_name, contact_studid, contact_phone, contact_email, contact_desc) VALUES (NULL, ?, ?, ?, ?, ?)";

        $stmt = mysqli_prepare($conn, $sql);

        mysqli_stmt_bind_param($stmt, "sssss", $contact_name, $contact_studid, $contact_phone, $contact_email, $contact_desc);
...
<body>
   <nav class="navbar navbar-light justify-content-center fs-3 mb-5" style="background-color: #cabbcb;">
     Contact Us Form for Book Worm Club
   </nav>
